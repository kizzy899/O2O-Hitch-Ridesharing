...
public class RabbitConfig {
    ...
    public Queue strokeOverQueue() {
        ...
        // x-message-ttl  声明队列的TTL,time to live 生存时间
        // 消息在队列中超过指定时间后会自动转移到死信队列
        ...
        return QueueBuilder.durable(STROKE_OVER_QUEUE)
                .withArguments(args)
                .build();
    }
     ...
     */
    @Bean
    Binding bindingStrokeOverDirect(Queue strokeOverQueue, DirectExchange strokeOverQueueExchange) {
     ...
     */
    @Bean
    Binding bindingStrokeDeadDirect(DirectExchange strokeDeadQueueExchange,Queue strokeDeadQueue) {
