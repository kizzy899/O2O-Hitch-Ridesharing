...
import static com.heima.commons.constant.HtichConstants.STROKE_DIAMETER_RANGE;
...
//        kafkaTemplate.send(HtichConstants.STROKE_START_GEO, JSON.toJSONString(worldMapBO));
    ...
    public ResponseVO<StrokeVO> inviteAccept(StrokeVO strokeVO) {
        ...
        //创建邀请状态
        int status = strokeVO.getStatus(); // 1
    ...
    //构建装饰者
    public static final Valuation a = new BasicValuation(null);
    public static final Valuation b = new StartPriceValuation(a);
    private static final Valuation c = new FuelCostValuation(b);
    /**
     * 添加订单
     *
    ...
    private void addOrder(StrokePO inviter, StrokePO invitee) {
        ...
        if (null != resultBO) {
            ...
            orderPO.setCost(c.calculation(orderPO.getDistance()/1000));
        }
    ...
    private void unbindStroke(StrokeVO strokeVO) {
        ...
        //同时删除相关司机的距离数据
//        List<ZsetResultBO> zsetResultBOList1 = redisHelper.getZsetSortVaues(HtichConstants.STROKE_GEO_ZSET_PREFIX, unbindId);
//        for (ZsetResultBO zsetResultBO : zsetResultBOList1) {
//            //删除司机的距离数据中关于此乘客的记录
//            redisHelper.delHash(HtichConstants.STROKE_GEO_DISTANCE_PREFIX, zsetResultBO.getValue(), unbindId);
//        }
    }
    ...
    private StrokeVO renderStrokeVO(StrokeVO strokeVO) {
        ...
        if (StringUtils.isNotEmpty(distanceStr) && distanceStr.contains(":")) {
            ...
            strokeVO.setStartDistance(Float.parseFloat(distances[0])); // 渲染起点距离
            strokeVO.setEndDistance(Float.parseFloat(distances[1])); // 渲染终点距离
        }

        //获取用户对象
        AccountPO accountPO = accountAPIService.getAccountByID(strokeVO.getPublisherId());
        if (null != accountPO) {
            strokeVO.setUseralias(accountPO.getUseralias()); // 渲染用户真实性命
            strokeVO.setAvatar(accountPO.getAvatar()); // 渲染用户头像
        }
        return strokeVO;
    }
    ...
    private Collection<HitchGeoBO> initGeoData(StrokePO strokePO) {
        ...
        for (HitchGeoBO hitchGeoBO : hitchGeoBOList) {
            //所有行程记录我
            // Redis
            // Key: STROKE_GEO_ZSET_ + 行程ID
            // Value: 与该行程匹配的其他行程ID
            // Score: 匹配分数(通过getScore方法计算)
            // 用途: 按匹配度对行程进行排序，分数越高匹配度越好
            redisHelper.addZset(HtichConstants.STROKE_GEO_ZSET_PREFIX, hitchGeoBO.getTargetId(), strokePO.getId(), getScore(hitchGeoBO));
            // Key: STROKE_GEO_DISTANCE_ + 行程ID
            // Field: 与该行程匹配的其他行程ID
            // Value: 起点距离和终点距离(通过getDistanceStr方法生成，格式为"起点距离:终点距离")
    ...
    private Collection<HitchGeoBO> geoFilterMatch(StrokePO strokePO) {
        ...
        //开始geo数据, 筛选出来半径100km内匹配的司机行程/乘客行程,开始行程
        /**
         * 匹配的行程id
         * 经纬度 + 行程id + 距离
         */
        ...
        //结束的geo数据，筛选出来半径100km内匹配的司机行程/乘客行程,结束行程
        ...
        //定义map key的set
        Set<String> startSet = startGeoBOMap.keySet();
        Set<String> endSet = endGeoBOMap.keySet();
    ...
    private float getScore(HitchGeoBO hitchGeoBO) {
         ...
         * 获取起点和终点距离，各占50%权重
         * 计算加权平均距离
         * 用1减去(加权距离/行程直径范围)得到匹配度
         * 乘以100转换为百分制分数
         */
        /**
         * 举例说明：
         * 假设系统找到了一个司机行程，该行程与乘客行程的起点和终点距离如下：
         * 起点距离：20公里
         * 终点距离：30公里
         * 1、计算加权平均距离：
         * 加权平均距离 = 起点距离 * 0.5 + 终点距离 * 0.5
         *                = 20 * 0.5 + 30 * 0.5
         *                = 10 + 15
         *                = 25公里
         * 2、计算相对距离比例：
         * 相对距离比例 = 加权平均距离 / 行程直径范围
         *                = 25 / 100
         *                = 0.25
         * 3、计算匹配度
         * 匹配度 = 1 - 相对距离比例
        ...
        /**
         * 第一步：算平均距离
         * 假设有一个司机，他的上车点离你20公里，下车点离你30公里：
         * 平均距离 = 20×0.5 + 30×0.5 = 25公里
         *
         * 第二步：算相对距离
         * 我们设定一个标准：100公里是最远的距离
         * 相对距离 = 25 ÷ 100 = 0.25
         *
         * 第三步：算匹配度
         * 距离越近匹配度越高，所以用1减去相对距离：
         * 匹配度 = 1 - 0.25 = 0.75
         *
         * 第四步：换算成百分制
         * 最终得分 = 0.75 × 100 = 75分
         * 这样，距离越近得分越高，系统就会推荐得分高的司机给你！
         */
        //思路：先算起点匹配度，再算终点匹配度，最后取平均
        double v = hitchGeoBO.getStartGeo().getDistance() * 0.5 + hitchGeoBO.getEndGeo().getDistance() * 0.5;
        double v1 = v / STROKE_DIAMETER_RANGE;
        // 距离越近，相对比例越小，匹配度越高
        double v2 = 1 - v1;
        return (float) (v2 * 100);
//        return ((float) (1 - (hitchGeoBO.getStartGeo().getDistance() * 0.5 + hitchGeoBO.getEndGeo().getDistance() * 0.5) / STROKE_DIAMETER_RANGE)) * 100;
    }
