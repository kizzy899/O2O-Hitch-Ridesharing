...
public class AccountHandler {
    ...
    public ResponseVO<AccountVO> modifyPassword(AccountVO accountVO) {
        ...
        //TODO:任务1-修改密码-1day
        //获取当前用户在数据库里的信息
        AccountPO account = accountAPIService.getAccountByID(userid);
        //旧密码加密，对比数据库，防止输入错误
        if (!CommonsUtils.encodeMD5(accountVO.getPassword()).equals(account.getPassword())) {
            throw new BusinessRuntimeException(BusinessErrors.PASSWORD_ERROR, "旧密码输入错误");
        }
        //新密码加密，对比旧密码，不允许相同
        if (CommonsUtils.encodeMD5(accountVO.getNewPassword()).equals(accountVO.getPassword())) {
            throw new BusinessRuntimeException(BusinessErrors.PASSWORD_SAME, "新密码不能与旧密码相同");
        }
        //校验通过，将新密码写入数据库，修改成功
        account.setPassword(CommonsUtils.encodeMD5(accountVO.getNewPassword()));
     ...
     * @return
     */
    public ResponseVO<AuthenticationVO> identityAuth() throws Exception {
        AccountPO accountPO = getCurrentAccountPO();
        AuthenticationPO authenticationPO = getAuthenticationPO(accountPO); // TODO FH
        if (null == authenticationPO) {
            throw new BusinessRuntimeException(BusinessErrors.DATA_NOT_EXIST, "用户认证信息不存在");
        }
        ...
        aiHelper.getLicense(authenticationPO);
